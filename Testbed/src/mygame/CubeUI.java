...
class CubeUI extends JPanel {
    ...
    public CubeUI(MainSwingCanvas canvasApplication, Callback onClick) {
        ...
        addCube.addActionListener( e -> {
            if(getWorld() != null) {
                getWorld().generateCube(Float.parseFloat(xField.getText()), Float.parseFloat(yField.getText()), Float.parseFloat(zField.getText()), getWorld().generateRandomSuitableColor());
                xField.setText("");
                yField.setText("");
                zField.setText("");
                onClick.run();
